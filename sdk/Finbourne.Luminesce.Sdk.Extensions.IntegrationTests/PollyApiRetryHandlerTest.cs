...
namespace Finbourne.Luminesce.Sdk.Extensions.IntegrationTests
{
    ...
    public class PollyApiRetryHandlerTests
    {
        ...
        private const string ListenerUriPrefix = "http://localhost:4445/";
        private int _apiCallCount;

        private readonly BackgroundQueryProgressResponse _mockResponse = new BackgroundQueryProgressResponse(false, 
            0, 
            Model.TaskStatus.Canceled, 
            BackgroundQueryState.Cancelled, 
            "progress:cancelled", 
            new List<FeedbackEventArgs>(), 
            "query", 
            "queryName", 
            new List<Column>());
        ...
        public void CallApiMethod_WhenHttpStatusIs400AndRetryConditionIsNotSatisfied_ThrowsApiExceptionWithoutRetry()
        {
            ...
            var exception = Assert.Throws<ApiException>(
                () => _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistory("some_execution_id")
            ) ;
        ...
        public void CallApiMethod_WhenHttpStatusIs200AndRetryConditionIsNotSatisfied_NoPollyRetryIsTriggered()
        {
            ...
            var sdkResponse = _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistory("some_execution_id");
        ...
        public void CallApiMethod_WhenExceedsPollyRetries_NoFallbackPolicyDefined_DoesNotThrow_ReturnsEmptyResponse(int returnedStatusCode)
        {
            ...
            // Calling GetPortfolio or any other API triggers the flow that triggers polly
            var response = _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistory("some_execution_id");
        ...
        public void CallApiMethod_WhenApiResponseStatusCodeSatisfiesRetryCriteria_ConnectivityIssuesNoRetry_Throws()
        {
            ...
            Assert.That(exception.Message, Contains.Substring("Internal SDK error occurred when calling GetProgressOfHistory: An error occurred while sending the request"));
        }
       ...
       [Test]
        ...
        public void UseRateLimitPolicyNoRetryAfter_WhenCallingApiMethodHitsRateLimit_RetryUsesExponentialBackoff()
        {
            ...
            var sw = Stopwatch.StartNew();
            // Calling the API triggers the flow that triggers polly
            _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistory("code");
            sw.Stop();
        ...
        public async Task CallApiMethodAsync_WhenHttpStatusIs200AndRetryConditionIsNotSatisfied_NoPollyRetryIsTriggered()
        {
            ...
            var sdkResponse = await _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistoryAsync("some_execution_id");
        ...
        public void CallApiMethodAsync_AsyncPollyIsTriggered_ThrowsWithExceededCallsFallbackPolicy(int returnedStatusCode)
        {
            ...
            var exception = Assert.ThrowsAsync<ApiException>(
                 () => _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistoryAsync("some_execution_id"));

            Assert.That(exception.Message, Is.EqualTo($"Error calling GetProgressOfHistory: {expectedErrorResponse}"));
            Assert.That(exception.ErrorCode, Is.EqualTo(returnedStatusCode));
            Assert.That(_apiCallCount, Is.EqualTo(expectedNumberOfApiCalls));
        }
        ...
        public async Task CallApiMethodAsync_AsyncPollyIsTriggered_NoFallbackPolicy_ReturnsNullResponseOnRetriesExceeded(int returnedStatusCode)
        {
            ...
            // Calling GetPortfolio or any other API triggers the flow that triggers polly
            var response = await _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistoryAsync("some_execution_id");
  ...
  [Test]
        ...
        public async Task UseRateLimitPolicyNoRetryAfterAsync_WhenCallingApiMethodHitsRateLimit_RetryUsesExponentialBackoff()
        {
            ...
            var sw = Stopwatch.StartNew();
            // Calling API triggers the flow that triggers polly
            await _apiFactory.Api<HistoricallyExecutedQueriesApi>().GetProgressOfHistoryAsync("code");
            sw.Stop();
            Assert.That(sw.ElapsedMilliseconds, Is.GreaterThanOrEqualTo(1000*(2+4+8))); // exponential backoff
            Assert.That(_apiCallCount, Is.EqualTo(expectedNumberOfApiCalls));
        }
