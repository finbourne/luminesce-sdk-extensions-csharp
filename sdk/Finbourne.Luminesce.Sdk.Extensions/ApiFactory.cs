...
namespace Finbourne.Luminesce.Sdk.Extensions
{
    ...
    public class ApiFactory : IApiFactory
    {
        ...
        public ApiFactory(ApiConfiguration apiConfiguration)
        {
            ...
            if (!Uri.TryCreate(apiConfiguration.LuminesceUrl, UriKind.Absolute, out var _))
            {
                if (string.IsNullOrWhiteSpace(apiConfiguration.LuminesceUrl))
                    throw new ArgumentNullException(
                        nameof(apiConfiguration.LuminesceUrl),
                        $"Luminesce Uri missing. Please specify either FBN_LUMINESCE_API_URL environment variable or luminesceUrl in secrets.json.");

                throw new UriFormatException($"Invalid Uri: {apiConfiguration.LuminesceUrl}");
            }

            // Create configuration
            var configuration = new TokenProviderConfiguration(tokenProvider)
            {
                BasePath = apiConfiguration.LuminesceUrl,
            };
